...
function scard.savefilter(c,tp)
	return c:IsLocation(DM_LOCATION_BATTLE) and c:IsFaceup() and c:IsDMRace(DM_RACE_ARMORED_DRAGON)
		and c:IsControler(tp) and not c:IsReason(REASON_REPLACE)
end
function scard.savetg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return eg:FilterCount(scard.savefilter,c,tp)==1 and not c:IsStatus(STATUS_DESTROY_CONFIRMED) end
